...
export default function ActualitesClient({ items, locale, introText, introHighlight, introBody }) {
  ...
  return (
    <div className="w-[88%] mx-auto space-y-8">
      ...
      {/* ── Featured ── */}
      <div className="relative flex items-center" style={{ height: '400px' }}>

        {/* Image — full height, right side, rounded */}
        ...
        {/* Text card — 80% height, left side, overlaps image slightly */}
        <div
          key={animKey}
        ...
        >
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-4 leading-snug line-clamp-2">
              {item.title}
            </h1>
            <p className="text-gray-700 text-[0.9375rem] leading-relaxed line-clamp-4 text-justify">
              {summary}
            </p>
          </div>
          {item.slug?.current && (
      ...
      <div className="relative">
        <div className="overflow-hidden">
          <div
            ...
            style={{ gap: '16px', transform: `translateX(calc(-${cardStart} * (100% / 3 + 16px / 3)))` }}
          >
            {items.map((n, i) => (
              <NewsPickCard
                key={n._id}
        ...
        {items.length > perPage && (
          <div className="flex items-center justify-center gap-3 mt-5">
            <NavBtn onClick={() => setCardStart((s) => Math.max(0, s - 1))} disabled={cardStart === 0} dir="prev" />
            {Array.from({ length: max + 1 }).map((_, i) => (
              <button key={i} onClick={() => setCardStart(i)}
                className={`w-2.5 h-2.5 rounded-full transition-all duration-200 ${i === cardStart ? 'bg-primary scale-125' : 'bg-gray-200 hover:bg-gray-300'}`}
                aria-label={`Page ${i + 1}`}
              />
            ))}
            <NavBtn onClick={() => setCardStart((s) => Math.min(max, s + 1))} disabled={cardStart >= max} dir="next" />
          </div>
        )}
      </div>
    </div>
...
function NewsPickCard({ item, locale, isSelected, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative flex-shrink-0 text-left rounded-xl overflow-hidden flex flex-col transition-all duration-200 hover:shadow-lg ${
        isSelected
          ? 'bg-white shadow-md border border-gray-200'
          : 'bg-gray-50 border border-gray-200 hover:bg-white hover:shadow-md'
      }`}
      style={{ width: 'calc(33.333% - 11px)', minHeight: '220px' }}
    >
      {/* Selected top border */}
      {isSelected && (
      ...
      {!isSelected && (
        <div className="absolute top-0 left-0 h-[3px] w-0 bg-primary/60 rounded-t-xl transition-all duration-500 ease-in-out group-hover:w-full" />
      )}

      <div className="px-5 pt-6 pb-5 flex flex-col flex-1">
        <p className="text-primary text-[11px] font-semibold mb-2 uppercase tracking-wide">
          {formatDate(item.date, locale)}
        </p>
        <h3 className="font-bold text-gray-900 text-sm leading-snug mb-2 line-clamp-2">
          {item.title}
        </h3>
        <p className="text-gray-600 text-xs leading-relaxed line-clamp-3 text-justify flex-1">
          {excerpt}
        </p>
        <span className="text-primary text-xs font-semibold mt-3 inline-block">
          Voir plus →
        </span>
      </div>
    </button>
